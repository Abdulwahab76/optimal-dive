...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TABLE "home_page_services_service_items" ALTER COLUMN "service_description" SET DATA TYPE jsonb;
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   ALTER TABLE "home_page_services_service_items" ALTER COLUMN "service_description" SET DATA TYPE varchar;
  ALTER TABLE "home_page_services_service_items" ALTER COLUMN "service_description" DROP DEFAULT;
  ALTER TABLE "home_page_services_service_items" ALTER COLUMN "service_description" SET NOT NULL;
  ALTER TABLE "home_page" ALTER COLUMN "about_about_body_text" SET DEFAULT '{"root":{"type":"root","children":[{"type":"paragraph","children":[{"type":"text","text":"built on one simple belief: great software should solve real problems and drive lasting business growth. We combine creative thinking with deep technical expertise to turn ambitious ideas into products that generate a profitable, sustainable impact.","version":1}],"version":1}],"direction":"ltr","format":"","indent":0,"version":1}}'::jsonb;`)
}
