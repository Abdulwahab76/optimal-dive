...
const AutoHeader = ({ autoHero }: { autoHero?: AutomotivePage["autoHero"] }) => {
  const stats = autoHero?.autoHeroStats?.length ? autoHero.autoHeroStats : defaultStats;
  const heroImage = mediaUrl(autoHero?.autoHeroImage, "/images/mobile-screen.png");
    ...
    <div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-32 lg:py-40 lg:h-screen flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <div className="p-2 px-4 mx-auto lg:mx-0 rounded-full border border-primary-1 w-fit flex items-center gap-2 mb-4">
              <Image src="/images/gemini.png" width={60} height={60} alt="gemini" className="h-auto w-auto" />
              <p className="font-medium text-sm uppercase">
                {autoHero?.autoHeroBadgeLabel ?? "Brand Identity Design"}
              </p>
            </div>
            <h1 className="text-3xl lg:text-6xl font-bold mb-4 leading-[1.15] text-center lg:text-left">
              {autoHero?.autoHeroTitleLine1 ?? "Automotive Software"}
              <br />
            ...
            >
              {autoHero?.autoHeroCtaLabel ?? "Start Your Project"}
              <ArrowRight size={15} />
            </Link>
          </div>
          <div className="relative top-20 lg:block hidden">
            <Image src={heroImage} alt="verified" width={500} height={500} />
          </div>
        </div>
        <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-none">
          <svg viewBox="0 0 1440 140" preserveAspectRatio="none" className="block h-28 w-full">
            <path fill="#ffffff" d="M0,40 C360,120 1080,120 1440,40 L1440,140 L0,140 Z" />
          </svg>
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 gap-y-14 md:grid-cols-4">
        {stats.map((item, index) => (
          <div
            key={item.autoStatValue + index}
            className={`text-center transition-all ${index === 1 ? "md:mt-12" : ""} ${index === 2 ? "md:mt-12" : ""} ${index === 3 ? "md:mt-0" : ""}`}
          >
            <h3 className="text-3xl font-semibold leading-none md:text-6xl">{item.autoStatValue}</h3>
